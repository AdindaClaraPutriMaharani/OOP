document.addEventListener('DOMContentLoaded', function () {
    ...
    // Tambahkan event listener untuk createTransactionForm
    createTransactionForm.addEventListener('submit', async function (e) {
        e.preventDefault();
        const loanId = document.getElementById('transaction-loan-id').value;
        const transactionType = document.getElementById('transaction-type').value;
        const transactionDate = document.getElementById('transaction-date').value;
        try {
            await axios.post('/transactions', { loan: { id: loanId }, transactionType, transactionDate });
    ...
    async function fetchAndDisplayData() {
        try {
            const [booksResponse, membersResponse, loansResponse, transactionsResponse] = await Promise.all([ // Tambahan
                axios.get('/books'),
                axios.get('/members'),
                axios.get('/loans'),
                axios.get('/transactions') // Tambahan
            ]);
            ...
            transactionsList.innerHTML = ''; // Tambahan
            transactionsResponse.data.forEach(transaction => { // Tambahan
                const li = document.createElement('li');
                li.textContent = `ID: ${transaction.id}, Loan ID: ${transaction.loan.id}, Transaction Type: ${transaction.transactionType}, Transaction Date: ${transaction.transactionDate}`;
                li.classList.add('list-group-item');
